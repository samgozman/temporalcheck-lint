...
func TestNolintForWorkeroptions(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:workeroptions", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForWorkeroptions(tt.text); got != tt.want {
				t.Errorf("nolintForWorkeroptions(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
