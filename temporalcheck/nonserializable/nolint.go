...
// nolintForNonserializable reports whether a comment is a golangci-lint nolint
...
// this analyzer by -- not the analyzer name "nonserializable"), ignoring any
// trailing "// explanation".
func nolintForNonserializable(text string) bool {
...
// nolintInfo records where nonserializable nolint directives appear in a file.
...
// collectNolint scans the file's comments for nonserializable nolint directives.
func collectNolint(fset *token.FileSet, file *ast.File) nolintInfo {
	info := nolintInfo{lines: make(map[int]bool)}

	for _, group := range file.Comments {
		for _, c := range group.List {
			if !nolintForNonserializable(c.Text) {
				continue
			}
...
// anchors (the target reference) including calls written across several lines.
