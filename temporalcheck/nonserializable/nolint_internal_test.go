...
func TestNolintForNonserializable(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:nonserializable", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForNonserializable(tt.text); got != tt.want {
				t.Errorf("nolintForNonserializable(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
