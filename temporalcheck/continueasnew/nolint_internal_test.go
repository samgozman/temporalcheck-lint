...
func TestNolintForContinueAsNew(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:continueasnew", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForContinueAsNew(tt.text); got != tt.want {
				t.Errorf("nolintForContinueAsNew(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
