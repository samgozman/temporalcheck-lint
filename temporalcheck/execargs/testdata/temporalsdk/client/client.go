...
// and SignalWithStartWorkflow on it), so a method's receiver is client.Client --
// not internal.Client. The stub mirrors that exactly so the analyzer's receiver
// match is exercised the way it resolves against the real SDK. The workflow target
// and its arguments are interface{}, which is the type erasure the analyzer
// resolves by inspecting the target's real signature instead.
package client
