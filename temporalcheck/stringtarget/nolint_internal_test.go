...
func TestNolintForStringtarget(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:stringtarget", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForStringtarget(tt.text); got != tt.want {
				t.Errorf("nolintForStringtarget(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
