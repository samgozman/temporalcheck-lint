...
func TestNolintForSensitiveargs(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:sensitiveargs", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForSensitiveargs(tt.text); got != tt.want {
				t.Errorf("nolintForSensitiveargs(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
