...
func TestNolintForActivitytimeout(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:activitytimeout", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForActivitytimeout(tt.text); got != tt.want {
				t.Errorf("nolintForActivitytimeout(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
