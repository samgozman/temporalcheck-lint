...
func TestNolintForLossynumber(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:lossynumber", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForLossynumber(tt.text); got != tt.want {
				t.Errorf("nolintForLossynumber(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
