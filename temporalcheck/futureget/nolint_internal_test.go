...
func TestNolintForFutureget(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:futureget", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForFutureget(tt.text); got != tt.want {
				t.Errorf("nolintForFutureget(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
