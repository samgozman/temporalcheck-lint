...
func TestNolintForOptionsdiscard(t *testing.T) {
	tests := []struct {
		name string
		text string
		want bool
		...
		{"names the analyzer, not the plugin", "//nolint:optionsdiscard", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := nolintForOptionsdiscard(tt.text); got != tt.want {
				t.Errorf("nolintForOptionsdiscard(%q) = %v, want %v", tt.text, got, tt.want)
			}
		})
	}
}
